...
namespace EPS.Utils.Service
{
    public class BaseService : IBaseService
    {
        ...
        public virtual async Task<TDto> FindAsync<TEntity, TDto>(object id)
            where TEntity : class
        {
            return _mapper.Map<TEntity, TDto>(await _repository.FindAsync<TEntity>(id));
        }
        ...
        public virtual IQueryable<TDto> FilterEntity<TEntity, TDto>(params Expression<Func<TEntity, bool>>[] predicates)
            where TEntity : class
        {
            return _repository.Filter(predicates).ProjectTo<TDto>(_mapper.ConfigurationProvider);
        }
        ...
        public virtual PagingResult<TDto> FilterEntityPaged<TEntity, TDto>(PagingParams<TEntity> pagingParams, params Expression<Func<TEntity, bool>>[] predicates)
            where TEntity : class
        {
            if (pagingParams == null)
            {
            ...
            IQueryable<TEntity> query = _repository.Filter<TEntity>();
            ...
            result.Data = query.ProjectTo<TDto>(_mapper.ConfigurationProvider).ToList();

            return result;
        }

        public async virtual Task<PagingResult<TDto>> FilterEntityPagedAsync<TEntity, TDto>(PagingParams<TEntity> pagingParams, params Expression<Func<TEntity, bool>>[] predicates)
            where TEntity : class
        {
            if (pagingParams == null)
            {
            ...
            result.Data = await query.ProjectTo<TDto>(_mapper.ConfigurationProvider).ToListAsync();
