                            ...
                            <b-col md="6">
                                <b-form-group label="Địa bàn:"
                                    ...
                                    <b-form-input type="text" style="display: none"
                                                  ...
                                                  :state="$v.unit.provinceId.$dirty ? !$v.unit.provinceId.$error : null"></b-form-input>
                                    <select2 style="width:100%" v-if="this.listProvinces.length > 0 || flag"
                                             placeholder="--- Chọn tỉnh/thành phố ---" id="region"
                                             aria-describedby="regionFeedback"
                                             v-model="$v.unit.provinceId.$model"
                                             allowClear="true"
                                             v-bind:options="listProvinces"></select2>
                                    <div style="margin:10px 0 10px 0;">
                                        <select2 style="width:100%;" v-if="this.listDistricts.length > 0 || flag"
                                                 placeholder="--- Chọn quận/huyện ---"
                                                 v-model="unit.districtId"
                                                 allowClear="true"
                                                 v-bind:options="listDistricts"></select2>
                                    </div>
                                    <select2 style="width:100%" v-if="this.listWards.length > 0 || flag"
...
<script>
    ...
    export default {
        ...
        computed: {
            flag() {
                //neeus ban ghi chua co region
                if (this.unit.regionId == null) {
                    console.log('vao');
                    ...
                    //ban ghi co roi
                    if (this.unit.districtId == null) {//quan huyen null thif show
                        this.districtFlag = true;
                    } else {
                        this.districtFlag = false;
                    }
                    if (this.unit.wardId == null) {//xa phuong null thi show
                        this.wardFlag = true;
                    }
                    else {
                        this.wardFlag = false;
                ...
                return this.unitFlag && this.provinceFlag && this.districtFlag && this.wardFlag;
            },
            unitId() {
        ...
        methods: {
            ...
            save() {
                ...
                if (!this.$v.$invalid) {
                    var vm = this;
                    this.$services.put(`master-data/units/${this.unitId}`, this.unit).done(() => {
                        vm.$toastr.s('Cập nhật đơn vị thành công');
                        window.UnitSelected = this.unitId;
                        vm.cancel();
            ...
            loadProvinces() {
                var vm = this;
                this.$services.get(`lookup/regions/null/null/list`, null).done((data) => {
            ...
            loadDistricts() {
                ...
                if (this.unit.provinceId) {
                    this.$services.get(`lookup/regions/${this.unit.provinceId}/null/list`, null).done((data) => {
            ...
            loadUnitDetail() {
                var vm = this;
                return this.$services.get(`master-data/units/${this.unitId}`).done(unit => {
                    vm.unit = unit;
                    vm.unitFlag = true;
                });
            },
            checkExitCode() {
                if (this.unit.code != null || this.unit.code != "") {
                    var vm = this;
                    var unit = store.getters.checkExitingCode(this.unitId,this.unit.code);
                    if (unit) {
                        this.codeUnique = "";
                    }
                    else {
