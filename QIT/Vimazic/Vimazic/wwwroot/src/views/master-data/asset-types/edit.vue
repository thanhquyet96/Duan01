    ...
    <div class="animated fadeIn">
        <b-row>
            <b-col lg="12">
                <b-card>
                    <div slot="header">
                        <strong>Cập nhật thông tin nhóm tài sản</strong>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            assetTypeId() {
                return this.$route.params.assetTypeId;
            },
            ...mapState({
        ...
        created() {
            this.loadAssetTypeDetail();
        },
        methods: {
            loadAssetTypeDetail() {
                var vm = this;
                return this.$services.get(`master-data/asset-types/${this.assetTypeId}`).done(assetType => {
            ...
            save() {
                ...
                if (!this.$v.$invalid) {
                    var vm = this;
                    this.$services.put(`master-data/asset-types/${this.assetTypeId}`, this.assetType).done((id) => {
                        vm.$toastr.s('Cập nhật nhóm tài sản thành công');
                        this.cancel();
